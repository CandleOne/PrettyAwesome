...
function deleteCloudinaryResourcesByPrefix(prefix, options) {
  return new Promise((resolve, reject) => {
    cloudinary.api.delete_resources_by_prefix(prefix, options, (error, result) => {
      if (error) reject(error);
      else resolve(result);
    });
  });
...
async function deleteCatalogResources() {
  const prefix = `${config.cloudinary.folder.replace(/\/$/, '')}/`;
  let deleted = 0;
  let nextCursor;
  do {
    const result = await deleteCloudinaryResourcesByPrefix(prefix, {
      resource_type: 'image',
      type: 'upload',
      invalidate: true,
      next_cursor: nextCursor,
