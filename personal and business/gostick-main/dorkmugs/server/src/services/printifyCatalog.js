...
async function runSync(products, options = {}) {
  if (syncPaused) {
    return { total: products.length, attempted: 0, created: 0, updated: 0, replaced: 0, failed: 0, results: [], paused: true };
  }
  ...
  activeSync = (async () => {
    ...
    if (options.onProgress) options.onProgress({ completed: 0, total: selected.length });
    for (const [index, product] of selected.entries()) {
      ...
      if (options.onProgress) {
        options.onProgress({ completed: index + 1, total: selected.length });
      }
    }
    return {
